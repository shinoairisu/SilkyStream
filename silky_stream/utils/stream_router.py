...
r = Router(root="router",{
    ...
    "/":{"com":组件类名,"参数名":{"required":True,"default":默认值,"type":int}} # 默认路由
...
class Router(object):
    ...
    Router
    """
    def __init__(self,root:str,router_info:dict):
        """
        root指的是URL中用来标定路由连接的参数
        http://localhost:8080/?router=hello
        如果后面没有路由参数，前往的就是标记为 / 的路由
        """
        self.root = root
        self.router_info = router_info

    async def __call__(self, slot=None):
        root = st.query_params[self.root]
        query = root if root else "/"
